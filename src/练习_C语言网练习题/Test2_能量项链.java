 ...
 * 710
 * @author: yrq
 * @create: 2020-02-19 16:11
 **/
public class Test2_能量项链 {
...
//        int j=1;
//        for (int i = 0; i < n-1; i++) {
//            ans+=arr[flagOfMax] * arr[(flagOfMax+j)%n] * arr[(flagOfMax+j+1)%n];
//            j++;
//        }
//        System.out.println(ans);
        for(int len=2;len<=2*n;len++)//倍增成环区间dp
        {
            for(int i=1;i<=2*n-len+1;i++)
            {
                int j=i+len-1;
                for(int k=i;k<j;k++)//枚举合并点
                    dp[i][j]=Math.max(dp[i][j],dp[i][k]+dp[k+1][j]+a[i]*a[k+1]*a[j+1]);//第一个头乘第一个尾乘后一个尾
            }
        }
        int ans=0;
        for(int i=1;i<=n;i++)//环形起点不确定枚举起点
